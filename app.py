...
ENROLLMENT_POSE_BUCKETS = ("center", "left", "right", "down")
...
def build_report_days_for_user(user: User, month: int, year: int, db: Session) -> tuple[datetime, list[dict[str, Any]]]:
    ...
    by_day: dict[int, list[Attendance]] = {}
    for log in logs:
    ...
    def join_times(times: list[datetime]) -> str | None:
        if not times:
            return None
        labels: list[str] = []
        for ts in times:
    ...
    for d in range(1, last_day + 1):
        ...
        day_logs.sort(key=lambda l: l.timestamp or datetime.min)

        am_logins: list[datetime] = []
        am_logouts: list[datetime] = []
        pm_logins: list[datetime] = []
        pm_logouts: list[datetime] = []
        am_absent = False
        pm_absent = False
        ...
        for log in day_logs:
            ...
            et = infer_event_type(st, getattr(log, "event_type", None))
            session_name = normalize_session(getattr(log, "session", None), ts)

            if st == "Absent" and getattr(log, "session", None) is None:
                am_absent = True
                pm_absent = True
                continue
            ...
            if et == "auto_absent" and st in ABSENT_STATUSES:
                if session_name == "AM":
                    am_absent = True
            ...
            if et == "login" and st in in_statuses:
                if session_name == "AM":
                    am_logins.append(ts)
                elif session_name == "PM":
                    pm_logins.append(ts)
            ...
            if et == "logout" or st == "Logout":
                if session_name == "AM":
                    am_logouts.append(ts)
                elif session_name == "PM":
                    pm_logouts.append(ts)

        am_in = join_times(am_logins) if am_logins else ("Absent" if am_absent else None)
        pm_in = join_times(pm_logins) if pm_logins else ("Absent" if pm_absent else None)
        am_out = join_times(am_logouts)
        pm_out = join_times(pm_logouts)
...
async def api_recognize(request: Request, db: Session = Depends(get_db)):
    ...
    - Uses blink detection in the current frame for liveness.
            ...
            for res in face_results:
                ...
                if candidate:
                    ...
                    if user:
                        ...
                        result["verified"] = bool(res.blink_detected)
    ...
    if redis:
        verified_users = {
            int(face["user_id"]): now_ts
            for face in results
            if face.get("verified") and face.get("user_id") is not None
        }
        try:
...
async def api_attendance_record(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Prevent cross-user recording when a specific employee ID was entered on the kiosk.
        if expected_uid is not None and uid != expected_uid:
            return JSONResponse(
                {
                    'status': 'error',
                    'message': 'Detected face does not match the entered employee ID. Please try again.',
                },
                status_code=409,
            )
        ...
        # Security Check: rely on current recognition/blink pipeline before submission.
        now_ts = time.time()
